...
if os.path.exists(EXCEL_FILE):
    ...
    # Chuẩn hóa ngày và sắp xếp cũ -> mới
    ...
    fig.update_layout(template="plotly_white", height=600, yaxis=dict(title="Giá (Nghìn đồng VNĐ)"), hovermode="x unified", dragmode="zoom")
    fig.update_xaxes(fixedrange=False)
    fig.update_yaxes(fixedrange=False)

    st.plotly_chart(fig, use_container_width=True, config={'scrollZoom': True})

    # 4. Bảng chi tiết: Định dạng số và sắp xếp cũ -> mới
    ...
    cols_order = ["STT", "Mã", "Ngày", "Mở cửa", "Cao nhất", "Thấp nhất", "Đóng cửa", "KL", "Giá trung bình (30 ngày giao dịch gần nhất)"]
    col_names = {
        "Mã": "Mã CP", "Mở cửa": "Giá mở cửa", "Cao nhất": "Giá cao nhất", 
