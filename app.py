...
if os.path.exists(EXCEL_FILE):
    ...
    c2.metric("Giá MA30", f"{latest['Giá trung bình (30 ngày giao dịch gần nhất)']:,.2f} VND")
    c3.metric("Ngày cập nhật", latest['Ngày'].strftime('%d/%m/%Y'))

    # 3. Biểu đồ Nến Nhật + Volume + MA30
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.03, 
                        subplot_titles=('Biểu đồ Giá (Nến Nhật) & MA30', 'Khối lượng giao dịch'), 
                        row_width=[0.2, 0.7])
    ...
    # 4. Bảng chi tiết
    st.subheader("📋 Bảng tra cứu dữ liệu")
